...
describe('Printer presets', () => {
    ...
    it('Collision', () => {
        ...
        expect(output).toEqual(
            [
                ...
                EOL,
                EOL,
                `  ❌ FAILED  Recca0120\\VSCode\\Tests\\AssertionsTest > is_not_same${EOL}`,
                `Failed asserting that two arrays are identical.${EOL}`,
                ` Array &0 [${EOL}`,
                `-    'a' => 'b',${EOL}`,
                `-    'c' => 'd',${EOL}`,
                ...
                EOL,
                EOL,
                `  ❌ FAILED  Recca0120\\VSCode\\Tests\\AssertionsTest > failed${EOL}`,
                `Failed asserting that false is true.${EOL}`,
                EOL,
    ...
    it('Pretty', () => {
        const printer = new Printer(phpUnitXML, PRESET_PRETTY);
        const output = runLifecycle(printer);
