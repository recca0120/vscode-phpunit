...
class RtrimExpression implements Expression<string> {
    supports(node: AstNode): boolean {
        return node.kind === 'function_call_expression' && (node as CallNode).name === 'rtrim';
    }

    resolve(node: AstNode, context: Context): string | undefined {
        const s = context.resolve((node as CallNode).arguments[0]);
        return typeof s === 'string' ? s.trimEnd() : undefined;
    }
}

export const rtrimExpression = new RtrimExpression();
