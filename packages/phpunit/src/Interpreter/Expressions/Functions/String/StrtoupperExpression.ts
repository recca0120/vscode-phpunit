...
class StrtoupperExpression implements Expression<string> {
    supports(node: AstNode): boolean {
        return node.kind === 'function_call_expression' && (node as CallNode).name === 'strtoupper';
    }
...
export const strtoupperExpression = new StrtoupperExpression();
