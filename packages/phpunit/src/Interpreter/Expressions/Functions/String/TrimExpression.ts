...
class TrimExpression implements Expression<string> {
    supports(node: AstNode): boolean {
        return node.kind === 'function_call_expression' && (node as CallNode).name === 'trim';
    }
...
export const trimExpression = new TrimExpression();
